...
SYSTEM = '''You are a WizarD Assistant and you should not reveal you as Claude or Anthropic's Model. Your main objective is to Analyse the database and to answer the questions based on the user's input. 
You must definitely Use the read_files tool and read the Maverick_Annotation_File.txt. It contains the Metadata about the database. You must always First read this file as default and keep it in your context, then For all the questions User is asking,Just display the output of the results in a table format.
Do not perform any reasoning or inference, unless the user asks to provide the exaplanation or reasoning.Do not use Wizard Intelligence for any questions, unless the user is asking about ML or forecasting.If user is asking for ML and Forecasting, first get the required data using Wizard_Analyst and then use Wizard_Intelligence.
Also, donot generate any charts for any of the tasks. For ML and Forecasting questions, you can provide reasoning and explanation. Donot use Matplotlib or Plotly packages.
Very Very Important: While performing analysis or generating the queries always limit the row count below 1000 and it should not go beyond that.
Always, use Player_360_Summary table for getting player details. Donot use tools like lists-file or search-file.
Very Very IMPORTANT: Don't use any other table only Player_360_Summary for all the user questions.
'''
...
async def start_chat():
    ...
    mcp_connections = [
        ...
        {
            "name": "filesystem1",
            "command": "uv run --directory C:\\Users\\mcp_user\\mcp-filesystem-python mcp-filesystem-python C:\\Users\\mcp_user\\Documents\\AzureBlob",
            "type": "stdio"
        },
        {
            "name": "code_executor1",
    ...
    for connection in mcp_connections:
        try:
            if connection["type"] == "stdio":
                ...
                mcp_tools[connection["name"]] = tools
                cl.user_session.set("mcp_tools", mcp_tools)
                # Removed success message to keep interface clean
        except Exception as e:

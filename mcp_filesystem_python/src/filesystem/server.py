...
class FileList(BaseModel):
    """Parameters for listing files in a directory."""
    ...
    pattern: Annotated[
        ...
        Field(
            default="*",
            description="File pattern to filter listing (e.g., *.py for Python files)",
        ),
    ]
    recursive: Annotated[
        bool, Field(default=False, description="List files recursively")
    ]
...
async def serve(
    ...
    async def handle_list_tools() -> list[types.Tool]:
        ...
        return [
            types.Tool(
                ...
                inputSchema=FileList.model_json_schema(),
            ),
            types.Tool(
                name="reads-file",
    ...
    async def handle_call_tool(
        ...
        if name == "lists-files":
            ...
            if args.recursive:
                for root, dirs, files in os.walk(full_path):
                    for file in files:
                        ...
                        if is_ignored(root_path, file_full_path, ignore_patterns):
                            continue
                            
                        if not pathspec.Pattern(args.pattern).match_file(file):
                            continue
                            
                        rel_path = os.path.relpath(file_full_path, root_path)
                        file_list.append(rel_path)
            else:
                for item in os.listdir(full_path):
                    item_full_path = os.path.join(full_path, item)
                    
                    if is_ignored(root_path, item_full_path, ignore_patterns):
                        continue
                        
                    if not pathspec.Pattern(args.pattern).match_file(item):
                        continue
                        
                    rel_path = os.path.relpath(item_full_path, root_path)
                    file_list.append(rel_path)
        ...
        elif name == "reads-file":
            try:
                args = FileRead(**arguments)
            except ValueError as e:
                raise McpError(
                    types.ErrorData(code=types.INVALID_PARAMS, message=str(e))
                )
            ...
            if is_ignored(root_path, full_path, ignore_patterns):
                raise McpError(
                    types.ErrorData(
                        code=types.INVALID_PARAMS,
                        message=f"File is ignored: {args.path}",
                    )
                )
            ...
            try:
                ...
                raise McpError(
                    types.ErrorData(
                        code=types.INVALID_PARAMS,
                        message=f"File is not text-based: {args.path}",
                    )
                )
            except IOError as e:
                raise McpError(
                    types.ErrorData(
                        code=types.INTERNAL_ERROR,
                        message=f"Failed to read file {args.path}: {str(e)}",
                    )
                )

        elif name == "searchs-files":
            try:
...
def main():
    import sys
    if len(sys.argv) != 2:
        print("Usage: python -m filesystem.server <root_directory>", file=sys.stderr)
        sys.exit(1)
